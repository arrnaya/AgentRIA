...
1. `ens/register.py` — registers `agentria.eth` subnames on ENSv2 Sepolia, sets ENSIP-26 text records per the table above.
...
Report the actual resolved subnames with a live ENS app link once registered (e.g. `app.ens.domains/oracle.agentria.eth`) — that link is one of RIA's explicit on-chain verification points for judges, so it needs to actually resolve, not just exist in code.
